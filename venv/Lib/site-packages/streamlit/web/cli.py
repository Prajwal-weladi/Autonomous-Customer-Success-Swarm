...
def main_hello(**kwargs: Any) -> None:
    ...
    from streamlit.hello import frontend

    filename = frontend.__file__
    _main_run(filename, flag_options=kwargs)


@main.command("run")
@configurator_options
@click.argument("target", default="frontend.py", envvar="STREAMLIT_RUN_TARGET")
@click.argument("args", nargs=-1)
def main_run(target: str, args: list[str] | None = None, **kwargs: Any) -> None:
    """Run a Python script, piping stderr to Streamlit.

    If omitted, the target script will be assumed to be "frontend.py".

    Otherwise, the target script should be one of the following:
    - The path to a local Python file.
    - The path to a local folder where "frontend.py" can be found.
...
def main_init(directory: str | None = None) -> None:
    ...
    # Create frontend.py
    (project_dir / "frontend.py").write_text(
...
""",
    ...
    click.secho(f"streamlit run {rel_path_str}/frontend.py", fg="blue")

    if click.confirm("❓ Run the app now?", default=True):
        app_path = project_dir / "frontend.py"
        click.echo("\nStarting Streamlit...")
